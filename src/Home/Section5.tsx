...
function Section5() {
  return (
    <section
      id="why-choose-us"
      dir="rtl"
      className="relative mx-auto mb-[70px] mt-[40px] w-full max-w-[1240px] overflow-visible scroll-mt-28 px-4 sm:mb-[90px] sm:px-6 lg:mb-[120px] lg:px-0"
    >
      <div className="relative mx-auto max-w-[1240px]">
        <div className="mx-auto max-w-[980px] text-center">
          ...
          <p className="mx-auto mt-4 max-w-[920px] text-[15px] leading-[1.75] text-slate-200 sm:mt-5 sm:text-[15px] md:text-[20px] lg:mt-6 lg:text-[24px]">
        ...
        <div className="mt-12 grid grid-cols-1 gap-x-8 gap-y-12 sm:mt-14 sm:grid-cols-2 sm:gap-y-14 lg:mt-[78px] lg:grid-cols-3 lg:gap-[64px]">
          {features.map((feature) => (
            <article
              key={feature.title}
              className="glass-card elevated-card mx-auto w-full max-w-[360px] rounded-[24px] p-6 text-center sm:p-7"
            >
              <div className="mb-5 flex justify-center sm:mb-6">
                <IconBox iconClasses={feature.iconClasses} icon={feature.icon} />
              </div>
              <h3 className="text-[18px] font-extrabold leading-[1.2] text-white sm:text-[20px] md:text-[22px] lg:text-[24px]">
                {feature.title}
              </h3>
              <p className="mt-4 text-[13px] leading-7 text-slate-300 sm:mt-5 sm:text-[14px] sm:leading-8 md:text-[15px] lg:mt-6 lg:text-[16px]">
                {feature.description}
              </p>
            </article>
          ))}
